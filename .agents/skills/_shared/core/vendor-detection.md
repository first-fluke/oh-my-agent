...
| Codex CLI | `.codex/agents/{name}.toml` native custom agents when the runtime supports same-vendor dispatch, otherwise `oma agent:spawn` | Native return or JSON output |
