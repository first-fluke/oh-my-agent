...
const EXCLUDED_PATTERNS = [
  ...
  "node_modules/",
  ".remotion/",
  // Lockfile for the vendored Remotion project (gitignored): regenerated by
  // `oma video doctor` on the developer's machine, never shipped via the
  // manifest. package.json (the tracked source) still ships.
  "package-lock.json",
  ...
  ".agents/state/",
  // Safe-write backups (gitignored): snapshots the CLI writes before it
  // overwrites harness files at run time. Pure local state, never shipped.
  // Without this, regenerating on a used checkout sweeps backup-* files in.
  ".agents/backup/",
