...
function buildCodexAgentFile(
  ...
  const lines = [
    ...
    `developer_instructions = ${serializeTomlMultiline(
      formatAgentBody(body, variant.protocolPath),
    )}`,
  ];
...
export function installVendorAgents(
  ...
  const variantPath = join(sourceDir, ".agents", "agents", "variants", `${vendor}.json`);
  ...
  for (const definition of readAbstractAgentDefinitions(sourceDir)) {
    ...
    writeFileSync(join(destDir, generated.fileName), generated.content);
  }
}
