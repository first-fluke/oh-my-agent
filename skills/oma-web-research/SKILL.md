...
description: Web research with cited sources via the You.com MCP server.
  Searches the current web, reads URLs into clean text, and produces one-shot
  cited synthesis through `you-search`, `you-contents`, and `you-research` MCP
  tools. Works keyless (`profile=free`, basic search) or with `YDC_API_KEY` for
  the full tool set. Falls back to runtime-native search when the server is not
  connected. Use for web research, current-events questions, URL reading, cited
  synthesis, and source-grounded answers on runtimes without native web search.
---
