...
**RARDO v2.1 — エージェント別モデル**: `.agents/oma-config.yaml` で各エージェントに独自のモデルと `effort` を割り当てられます。プリセットは5種類の runtime profile: `claude-only`、`codex-only`、`gemini-only`、`antigravity`、`qwen-only`。解決後の auth マトリクスは `oma doctor --profile` で確認できます。完全ガイド: [web/docs/guide/per-agent-models.md](../web/docs/guide/per-agent-models.md)。
