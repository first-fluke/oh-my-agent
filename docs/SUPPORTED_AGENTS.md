...
> **RARDO v2.1 — slug-based dispatch.** Vendor selection is now driven by model slugs resolved against `CORE_REGISTRY` (12 verified slugs) plus user additions via `.agents/config/models.yaml`. Per-agent overrides and the active runtime profile live in `.agents/config/user-preferences.yaml`. See [web/docs/guide/per-agent-models.md](../web/docs/guide/per-agent-models.md).
