...
| **oma-web-research** | Веб-поиск с указанием источников через MCP-сервер You.com — бесплатный профиль без ключа и откат на нативный поиск |
...
| **Event-sourced состояние** | Каждое прохождение гейта, каждый провал и каждое решение добавляют одну JSON-строку в `.agents/state/sessions/{sid}/events.jsonl` с отметкой вендора и id сессии runtime. Только на добавление, кросс-вендорно, проверяемо после запуска. | [`event-spec.md`](../.agents/skills/_shared/runtime/event-spec.md) |
...
| **Harness оценки скилов** | `oma skills eval` измеряет прирост пользы на отложенных задачах — treatment против baseline — вместо того чтобы просто верить, что скил помогает. `oma skills opt` оставляет только те правки, которые улучшают измеренный прирост. | [гайд по skill-eval](../web/docs/guide/skill-eval.md) |
