...
**Хардкорный мультиагентный оркестратор для программного обеспечения производственного уровня.**
Это не просто очередная обертка для чата — oh-my-agent — это профессиональный инструмент, который дает вашему ИИ-ассистенту целую команду инженеров.
...
| **oma-explainer** | Превращает diff, PR или ветку в автономный интерактивный HTML-разбор с квизом |
...
- **Портативный**: `.agents/` путешествует с вашим проектом, не привязан к одной IDE. `oma emit` проецирует один и тот же SSOT в артефакты открытых стандартов — папки скилов, совместимые с [Agent Skills](https://agentskills.io/specification), файл `.claude-plugin/marketplace.json` и `AGENTS.md` — так что скилы oma работают в любом инструменте, читающем открытую спецификацию, а проверка дрейфа (drift) в CI держит сгенерированный вывод честным
