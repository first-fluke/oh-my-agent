...
**説明：** インタラクティブなプロジェクト設定。

**トリガーキーワード：** なし（明示的呼び出しのみ）。

**ステップ：** 言語設定 -> CLIインストール状態確認 -> MCP接続状態（SerenaのCommandモードまたはSSEモード）-> エージェント-CLIマッピング -> サマリー -> リポジトリへのスター付けの確認。

**出力：** `.agents/config/user-preferences.yaml`。
