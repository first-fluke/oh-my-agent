...
### Knowledge Base
에이전트 산출물이 `.agents/results/`에 저장됩니다. 기획서, 코드, 리포트, 조율 메모 포함.
...
3. **Knowledge Base 검토** — `.agents/results/`에서 API 일관성 확인
...
| 에이전트 간 코드 불일치 | `.agents/results/`에서 산출물을 검토하고, 한 에이전트를 다른 에이전트 출력 참조로 재실행한 뒤 QA Agent로 최종 일관성을 점검하세요 |
