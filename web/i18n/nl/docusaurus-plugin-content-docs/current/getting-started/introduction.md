...
| **oma-backend** | API & server-specialist | Clean architecture (Router-Service-Repository-Models). Stack-agnostisch — detecteert Python/Node.js/Rust/Go/Java/Elixir/Ruby/.NET uit projectmanifesten. JWT + Argon2id voor auth. Bronnen: `execution-protocol.md`, `orm-reference.md`, `examples.md`, `checklist.md`, `error-playbook.md`. Ondersteunt `/stack-set` voor het genereren van taalspecifieke `stack/`-referenties. |
