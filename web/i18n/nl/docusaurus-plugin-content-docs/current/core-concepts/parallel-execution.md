...
| `--vendor <naam>` | `-m` | Overschrijf CLI-leverancier voor deze specifieke spawn. Opties: `antigravity`, `claude`, `codex`, `qwen`. |
...
|----------|--------|---------|
| 1 (hoogste) | `--vendor`-vlag | `oma agent spawn backend "task" session-01 --vendor claude` |
...
De `--vendor`-vlag wint altijd. Zonder vlag kijkt het systeem naar `agents:`-overschrijvingen en daarna naar de presetstandaarden.
