...
**`oma-config.yaml`** — Centraal configuratiebestand met: `language` (antwoordtaalcode), `date_format` (tijdstempelnotatie), `timezone` (tijdzone-identificator), `default_cli` (fallback CLI-leverancier), `model_preset` (per-agent CLI-routering).
