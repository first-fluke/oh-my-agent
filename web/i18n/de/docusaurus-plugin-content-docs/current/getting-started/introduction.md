...
| **oma-backend** | API- und Server-Spezialist | Clean Architecture (Router-Service-Repository-Modelle). Stack-agnostisch — erkennt Python/Node.js/Rust/Go/Java/Elixir/Ruby/.NET aus Projektmanifesten. JWT + Argon2id für Authentifizierung. Ressourcen: `execution-protocol.md`, `orm-reference.md`, `examples.md`, `checklist.md`, `error-playbook.md`. Unterstützt `/stack-set` zur Generierung sprachspezifischer `stack/`-Referenzen. |
...
| **OpenCode** | Skills + In-Process-Plugin-Bridge + generierte Subagenten (`.opencode/agents/`) | `oma agent spawn --vendor opencode` |
