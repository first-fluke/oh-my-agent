...
外部AI CLI（agy、codex、claude、gemini、またはqwen）を使用してコードレビューを実行。
...
| フラグ | 説明 |
|:-----|:-----------|
| `-m, --model <vendor>` | 使用するCLIベンダー：`antigravity`、`codex`、`claude`、`gemini`、`qwen`。デフォルトは設定から解決されたベンダー。 |
...
- `agy`、`claude`、`gemini`、`qwen`の場合：レビュープロンプトを構成してCLIを呼び出し。
