...
| Agente | Rol | Capacidades Clave |
|--------|-----|-------------------|
| **oma-translation** | Traducción consciente del contexto | Método de traducción en 4 etapas: Analizar Fuente, Extraer Significado, Reconstruir en Idioma Destino, Verificar. Preserva tono, registro y terminología del dominio. Detección de patrones anti-IA. Soporta traducción por lotes (archivos i18n). Modo refinado opcional de 7 etapas para calidad de publicación. Los perfiles por idioma de destino (`resources/lang/{code}.md`) contienen los sistemas de registro, la tipografía y las reglas contra el lenguaje traducido propias de cada idioma. Recursos: `translation-rubric.md`, `anti-ai-patterns.md`, `lang/{ko,ja,zh,en}.md`. |
...
| **OpenCode** | Habilidades + puente de plugin en proceso + subagentes generados (`.opencode/agents/`) | `oma agent spawn --vendor opencode` |
