...
|----------|--------|---------|
| 1 (la más alta) | flag `--model` | `oma agent:spawn backend "task" session-01 -m claude` |
...
El flag `--model` siempre gana. Sin flag, el sistema consulta las sobrescrituras `agents:` y después los valores por defecto del preset.
