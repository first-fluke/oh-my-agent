...
설계는 두 연구 결과를 따릅니다. SkillOpt(arXiv:2605.23904)는 스킬 편집을 수락하는 게이트로 held-out 유용성 점수를 씁니다. SkillLens(arXiv:2605.23899)는 스킬 유용성이 설명의 구별성과 무관함을 보여줍니다. 구별되는 스킬도 쓸모없을 수 있고, 겹치는 스킬도 도움이 될 수 있습니다.
...
## mock과 live 점수에 대한 솔직한 참고

`assert`와 `regex` 체커를 쓰는 `--mock`은 결정론적 *측정 기반*입니다. 계약 검사(출력에 올바른 도구 호출이 들어 있는가?)에는 유용하지만, 열린 형태의 스킬에 대한 믿을 만한 유용성 수치는 아닙니다. 의미 있는 점수를 얻으려면 judge 체커와 함께 `--live`로 돌려야 하며, 이 기능의 바탕이 된 학술 벤치마크도 그렇게 유용성을 측정합니다(SkillOpt는 벤치마크 정확도를, SkillLens는 유용성 기반 태스크 성공률을 씁니다).
