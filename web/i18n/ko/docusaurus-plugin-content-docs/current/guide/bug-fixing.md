...
증상이 아닌 근본 원인을 진단한다. `user.id`가 undefined라면 "undefined를 어떻게 막을까"가 아니라 "이 실행 경로에서 user가 왜 undefined인가"를 먼저 물어야 한다.
...
프론트엔드의 에러가 백엔드 변경 때문에 발생하고, 그 변경은 데이터베이스 스키마 마이그레이션 때문에 발생한 경우. 근본 원인이 도메인 경계를 넘을 때 관련 도메인 에이전트를 포함하도록 `/work` 또는 `/orchestrate`로 에스컬레이션합니다.
