...
| **oma-backend** | Especialista em API e servidor | Arquitetura limpa (Router-Service-Repository-Models). Agnóstico de stack — detecta Python/Node.js/Rust/Go/Java/Elixir/Ruby/.NET a partir dos manifestos do projeto. JWT + Argon2id para auth. Recursos: `execution-protocol.md`, `orm-reference.md`, `examples.md`, `checklist.md`, `error-playbook.md`. Suporta `/stack-set` para geração de referências específicas de linguagem em `stack/`. |
