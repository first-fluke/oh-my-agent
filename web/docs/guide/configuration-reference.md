...
| `providers` | `docs`, `web`, `code_intelligence`, `code_intelligence_guard`, `semantic_memory` | `context7`, `native`, `serena`, `block`, `agentmemory` | Select documentation, search, code-intelligence, and semantic-memory providers. Code intelligence accepts `serena` or `gortex`; semantic memory accepts `agentmemory`, `honcho`, or `none`. `code_intelligence_guard` (`block` or `off`) controls the PreToolUse hook that denies native Grep/Glob/recursive shell search while a code-intelligence provider is configured; `OMA_CI_ALLOW_NATIVE=1` prefixed on a shell command bypasses it when the provider is down. |
